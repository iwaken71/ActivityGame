...
public class UnitychanLocalAI : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		if (ScoreScriptLocal.GAME) {
			direction = (destinaton - transform.position).normalized;
	...
	*/
	...
	void OnCollisionEnter(Collision col){
			if (col.gameObject.tag == "Tofu") {
				...
				if (Abs (n - col.gameObject.GetComponent<TofuScriptLocal> ().myColorNumber) > 0.01f) {
